...
class Admin extends CI_Controller {
        ...
        if($value != '') {
           ...
           // $eventphoto = $this->input->post('eventcoverphoto2');
            ...
            $data3 = array(
                ...
                'TicketPrice' => $this->input->post('ticketprice'),
                'AdditionalInformation' => $this->input->post('additionalinformation'),
                'EventCoverPhoto' =>  (!empty($eventphoto) ? (!empty($eventphoto) ? $eventphoto : $this->input->post('profileImage')) : (empty($_FILES['eventcoverphoto']['name']) ? $this->input->post('profileImage') : 'assets/img/events/' . $_FILES["eventcoverphoto"]["name"])),
                'RegistrationUrl' => $this->input->post('registrationurl'),
                //'StatusId' => '2',
                'EventCreator_UserId' => $this->session->userdata('user_id'),
                'CreatedAt' => date("Y/m/d"),
    ...
    public function update_event_status(){
        ...
        $emails = $this->user_model->fetch_user_newsletter_email($Country);

        foreach($emails as $email) {
            $from_email = "info@excelzweb.com";
            $to_email = $email['email'];

            //Load email library
            $this->load->library('email');

            $message = 'Your Event is approved which you have subscribed for';
            $this->email->from($from_email, 'Arthur');
            ...
            if($this->email->send()) {
                $status = AJAX_SUCCESS;
                $code = 200;
                $message_txt = 'An email has been sent to your email address'; }
            else {
                $message_txt .= 'Provided email address does not belong to any record.';  }
