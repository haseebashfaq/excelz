...
class User_model extends CI_model
{
    ...
    public function corporate_events(){
        $query = $this->db->query('Select * from event where EventCategoryId = 2 AND StatusId = 1 AND StartDate > CURDATE() order by id asc');
        return $query->result_array();
    }


    public function ib_events(){
        $query = $this->db->query('Select * from event where EventCategoryId = 3 AND StatusId = 1 AND StartDate > CURDATE() order by id asc');
        return $query->result_array();
    }


    public function all_events_search(){
        $query = $this->db->query('Select * from event where StatusId = 1 AND StartDate > CURDATE() order by id asc');
        return $query->result_array();
    }
    ...
    {
        $query = $this->db->query('select Id from speakers where EventId ='.$eventid);
        return $query->result_array();
    ...
    public function list_of_approved_events(){
        $query = $this->db->query("Select *, event.Id as eventid from event join eventtype ON eventtype.Id = event.EventTypeId JOIN eventcategory on event.EventCategoryId = eventcategory.Id join country ON country.Id = event.Country where event.StatusId = 1 AND event.StartDate > curdate() order by event.StartDate asc");
        return $query->result_array();
    }
    ...
    public function login_prouser($login)
    {
        $this->db->select('*');
        $this->db->from('user');
    ...
    public function search($event_name, $event_type, $country, $language, $startdate, $enddate){
        ...
        if ($userDataEntered) {
            $queryString = "SELECT * FROM event join eventcategory ON event.EventCategoryId = eventcategory.Id JOIN country ON country.Id = event.Country JOIN eventtype ON event.EventTypeId = eventtype.Id  JOIN language ON event.language_id = language.Id  {$eventWhere} {$eventWhere2}";
    ...
    public function upcoming_events(){
        ...
        $query = $this->db->query('select * from event where StartDate > CURDATE() AND StatusId = 1');
        return $query->result_array();
    ...
    public function get_by_event_id_user($id){
        $query = $this->db->query('Select * from event join speakers on event.Id = speakers.EventId join eventtype ON event.EventTypeId = eventtype.Id JOIN language ON event.language_id = language.Id where event.id = '.$id.'');
        return $query->result_array();
    }
